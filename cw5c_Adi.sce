...
// faza liczenia błędu tymczasowego podczas uczenia się sieci, dane podawane dla co setnej interacji
if (modulo(iteracja,100) == 0)
    for j=1:4
        ...
        tymczasowy_blad = tymczasowy_blad + abs(A(j,4) - U5);
        disp(tymczasowy_blad);
    end
    blad = tymczasowy_blad;
    tymczasowy_blad = 0;
    err(1,zmienna) = iteracja;
    err(2,zmienna) = blad;
    zmienna = zmienna + 1;
end
...
//wyświetlenie błedu tymczasowego
mtlb_axis([0 50000 0 3]);
plot(err(1,:),err(2,:),'b');

//sprawdzanie wyuczenia sieci, dane podawane z klawiatury przez użytkownika
